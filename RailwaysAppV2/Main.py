...
class WebcamApp:
    def __init__(self):
        ...
        self.maintitle = tk.Label(self.window, text = "Railway Protection Project", font = ('mooli', 40),
                                  fg = '#000870')
        self.maintitle.place(x = 100, y = 20)


        self.iml = tk.Label(self.window)
        self.iml.place(x = 0, y = 100)
        
        cx = 900

        self.analy_l = tk.Label(self.window, font = ("consolas", 30), fg = '#E54601', text = 'ANALYSIS:')
        self.analy_l.place(x = cx, y = 200)
        
        self.tex_queue = tk.Label(self.window, font=(font_type, 20))
        self.tex_queue.place(x = cx, y = 300)

        self.tex_pol = tk.Label(self.window, font=(font_type, 20))
        self.tex_pol.place(x = cx, y = 400)

        self.tex_tracks = tk.Label(self.window, font=(font_type, 20))
        self.tex_tracks.place(x = cx, y = 500)
    ...
    def update(self):
        ...
        if ret:
            ...
            try:
                ...
                if class_name == "0":
                    self.tex_queue.config(text='Normal Crowd', fg='green', font = (font_type, 18))
                else:
                    self.tex_queue.config(text='Open Queue Counters', fg='red', font = (font_type, 20, "bold"))
                ...
                if class_name == 'Pas':
                    self.tex_pol.config(text="RPF Required", fg = "red", font = (font_type, 20, "bold"))
                else:
                    self.tex_pol.config(text = 'Crowd under Control', fg = 'green', font = (font_type, 18))
                ...
                if class_inx == '0':
                    self.tex_tracks.config(text="Obstacles on Track", fg = "red", font = (font_type, 20, "bold"))
                else:
                    self.tex_tracks.config(text="Tracks Clear", fg = "green", font = (font_type, 18))
